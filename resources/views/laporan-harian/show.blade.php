<x-app>
    ...
    <div class="container px-6 mx-auto grid">
            ...
            <div class="lg:col-span-2">
                <x-card>
                    <div class="flex justify-between items-start mb-6">
                        ...
                        <span class="px-3 py-1 text-sm font-semibold leading-tight text-{{ $laporanHarian->status_badge_class }}-700 bg-{{ $laporanHarian->status_badge_class }}-100 rounded-full">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="text-gray-900 dark:text-gray-100 font-medium">{{ $laporanHarian->tanggal->format('d/m/Y') }}</div>
                        </div>
                        
                        <div>
                            <x-label>Waktu Laporan</x-label>
                            <div class="text-gray-700 dark:text-gray-200">{{ $laporanHarian->tanggal_laporan->format('H:i') }}</div>
                        </div>
                        
                        <div>
                            <x-label>Jenis Limbah</x-label>
                            <div class="text-gray-900 dark:text-gray-100 font-medium">{{ $laporanHarian->jenisLimbah->nama }}</div>
                            <div class="text-sm text-gray-500">{{ $laporanHarian->jenisLimbah->kode_limbah }}</div>
                        </div>
                        
                        <div>
                            <x-label>Kategori</x-label>
                            <div class="text-gray-700 dark:text-gray-200">{{ $laporanHarian->jenisLimbah->kategori_name }}</div>
                        </div>
                        
                        <div>
                            <x-label>Penyimpanan</x-label>
                            <div class="text-gray-900 dark:text-gray-100 font-medium">{{ $laporanHarian->penyimpanan->nama_penyimpanan }}</div>
                ...
                @if($laporanHarian->canSubmit())
                    <x-card class="mt-6">
                        ...
                        <div class="flex space-x-3">
                            <form action="{{ route('laporan-harian.submit', $laporanHarian) }}" method="POST" class="inline">
                                @csrf
                                <x-button type="submit" onclick="return confirm('Yakin ingin submit laporan ini? Laporan yang sudah disubmit tidak dapat diedit lagi.')">
                            ...
                            <a href="{{ route('laporan-harian.edit', $laporanHarian) }}" class="inline-flex items-center px-4 py-2 bg-yellow-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-700 active:bg-yellow-900 focus:outline-none focus:border-yellow-900 focus:ring ring-yellow-300 disabled:opacity-25 transition ease-in-out duration-150">
                                <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                                </svg>
                                Edit Laporan
                ...
                <x-card>
                    ...
                    <div class="space-y-3 text-sm">
                        <div class="flex justify-between">
                            ...
                            <span class="text-gray-900 dark:text-gray-100 font-medium">{{ $laporanHarian->jenisLimbah->kode_limbah }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600 dark:text-gray-400">Kategori:</span>
                            <span class="text-gray-900 dark:text-gray-100">{{ $laporanHarian->jenisLimbah->kategori_name }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600 dark:text-gray-400">Satuan:</span>
                            <span class="text-gray-900 dark:text-gray-100">{{ $laporanHarian->jenisLimbah->satuan_name }}</span>
                        </div>
                        ...
                        @if($laporanHarian->jenisLimbah->deskripsi)
                            <div class="pt-3 border-t border-gray-200 dark:border-gray-700">
                                ...
                                <p class="text-gray-700 dark:text-gray-300 text-xs mt-1">{{ $laporanHarian->jenisLimbah->deskripsi }}</p>
                            </div>
                        @endif
                    </div>
                </x-card>
                ...
                <x-card>
                    ...
                    <div class="space-y-3 text-sm">
                        <div class="flex justify-between">
                            ...
                            <span class="text-gray-900 dark:text-gray-100 font-medium">{{ $laporanHarian->penyimpanan->nama_penyimpanan }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600 dark:text-gray-400">Lokasi:</span>
                            <span class="text-gray-900 dark:text-gray-100">{{ $laporanHarian->penyimpanan->lokasi }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600 dark:text-gray-400">Jenis:</span>
                            <span class="text-gray-900 dark:text-gray-100">{{ $laporanHarian->penyimpanan->jenis_penyimpanan_name }}</span>
                        </div>
                        <div class="flex justify-between">
                        ...
                        <div class="pt-3 border-t border-gray-200 dark:border-gray-700">
                            ...
                            <div class="flex justify-between text-xs mt-1">
                                <span class="text-gray-500">{{ $laporanHarian->penyimpanan->persentase_kapasitas }}%</span>
                                <span class="px-2 py-1 text-{{ $laporanHarian->penyimpanan->status_kapasitas_color }}-700 bg-{{ $laporanHarian->penyimpanan->status_kapasitas_color }}-100 rounded">
                ...
                <x-card>
                    ...
                    <div class="space-y-3 text-sm">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="text-gray-900 dark:text-gray-100">{{ $laporanHarian->created_at->format('d/m/Y H:i') }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600 dark:text-gray-400">Diupdate:</span>
                            <span class="text-gray-900 dark:text-gray-100">{{ $laporanHarian->updated_at->format('d/m/Y H:i') }}</span>
                        </div>
                        @if($laporanHarian->isSubmitted())
                            <div class="flex justify-between">
                                <span class="text-gray-600 dark:text-gray-400">Disubmit:</span>
                                <span class="text-gray-900 dark:text-gray-100">{{ $laporanHarian->submitted_at?->format('d/m/Y H:i') ?? '-' }}</span>
                            </div>
                        @endif
                    </div>
                </x-card>
        ...
        @if($laporanTerkait = \App\Models\LaporanHarian::where('penyimpanan_id', $laporanHarian->penyimpanan_id)
            ...
            @if($laporanTerkait->count() > 0)
                <x-card class="mt-6">
                    <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-6">Laporan Terkait (Penyimpanan yang Sama)</h3>
