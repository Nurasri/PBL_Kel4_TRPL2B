        ...
        <x-card>
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <div class="mt-2">
                        @if($perusahaan->logo)
                            <img src="{{ Storage::url($perusahaan->logo) }}" alt="Logo {{ $perusahaan->nama_perusahaan }}" class="w-32 h-32 object-cover rounded-lg">
                        @else
                ...
                <div>
                    ...
                    <div class="text-gray-700 dark:text-gray-200">{{ $perusahaan->created_at->format('d F Y H:i') }}</div>
                </div>
                <div>
                    <x-label>Terakhir Diperbarui</x-label>
                    <div class="text-gray-700 dark:text-gray-200">{{ $perusahaan->updated_at->format('d F Y H:i') }}</div>
                </div>
            </div>
            <div class="flex items-center justify-end mt-8 space-x-2">
                @if(auth()->user()->id === $perusahaan->user_id || auth()->user()->isAdmin())
                    <a href="{{ route('perusahaan.edit', $perusahaan) }}" class="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 transition font-semibold">Edit Profil</a>
                @endif
                @if(auth()->user()->isAdmin())
                    <a href="{{ route('perusahaan.index') }}" class="px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700 transition font-semibold">Kembali</a>
                @endif
            </div>
        </x-card>
    </div>
